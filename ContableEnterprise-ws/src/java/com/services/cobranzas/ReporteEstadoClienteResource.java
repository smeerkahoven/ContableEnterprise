...
public class ReporteEstadoClienteResource extends TemplateResource {
     ...
     */
    public ReporteEstadoClienteResource() {
    }
    ...
    public RestResponse generarReporte (final RestRequest request) {
        RestResponse response = doValidations(request);
        if (response.getCode() == ResponseCode.RESTFUL_SUCCESS.getCode()){
            
            try {
                ReporteEstadoClienteSearchJson json = BeanUtils.convertToReporteEstadoCliente(request);
                json.setIdEmpresa(user.getIdEmpleado().getIdEmpresa().getIdEmpresa());
                
                List <ReporteEstadoClienteMainDto> l = ejbKardex.generarReporteEstadoCliente(json);
                if (l.isEmpty()) {
                ...
                Logger.getLogger(ReporteEstadoClienteResource.class.getName()).log(Level.SEVERE, null, ex);
            }
